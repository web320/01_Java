...
public class ArrayExample2 {
	...
	System.out.println("arr wnth : " + arr);
	System.out.println("copyArr1 주소 : " + copyArr1);
	
	System.out.println("arr 값: " + Array.toString(arr));
	System.out.println("copyArr 값: " + Arrays.toString(copyArr));
	
	//2. System.arraycopy(원본 배열명, 원본복사 시작 인덱스);
	// 					복사 배열명, 복사배열의 삽입 시작 인덱스,
	//					복사길이);
	...
	System.out.println("copyArr2 : " + Arrays.toString(copyArr));
	System.out.println("copyArr3 : " + Arrays.toString(copyArr));
	...
	public void createLottoNumber() {
		...
		int[] Lotto = new int[6];
		//2) 생성된 배열(lotto)을 처음부터 끝까지 순차접근하는 
		//for문 작성
		for(int i = 0; i < Lotto.length; i++) {
			
			//3) 1~ 45 사이 난수 생성
			int random = (int) (Math.random() * 45 + 1);
			// 번호가 45까지 있는데 1부터 시작하니깐???
			
			// 4) 생성된 난수를 순서대로 배열 요소에 대입
			Lotto[i] = random;
			// lotto[0] = 8; // [8,0,0,0,0,0]
			// lotto[1] = 10; // [8,10,0,0,0]
			// lotto[2] = 8; // [8,10,8,0,0]
